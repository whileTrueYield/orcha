...
import { debounce, get, indexOf, map, without } from "lodash";
...
export const TicketWorkflowStateChecklist: FCWithFragments<Props> = (props) => {
  const { className, ticketWorkflowState } = props;
  const { checklist } = ticketWorkflowState;

  // we want to maintain the items in a local state but we also want to
  // flush it when the checklist itself changes
  const [items, setItems] = useState(checklist || []);
  useEffect(
    () => setItems(get(ticketWorkflowState, "checklist", [])),
    [ticketWorkflowState, setItems]
  );
  ...
  const renderFormButton = () => (
    <button
      ...
      className="flex w-full flex-row items-center justify-center space-x-2 rounded-md px-2 py-1.5 text-sm font-medium transition hover:bg-gray-500 hover:bg-opacity-10"
    >
      ...
      return (
        <div className="space-y-1">
          {map(items, (item, index) => (
            <TicketChecklistItem
              checklistId={ticketWorkflowState.id}
              label={item.label}
              checked={item.checked}
              key={index}
  ...
  return <div className={className}>{renderItemsAndForm()}</div>;
};
...
TicketWorkflowStateChecklist.fragments.TicketChecklistFragment = gql`
  ...
  ${TicketWorkflowStateChecklist.fragments.TicketWorkflowStateChecklistFragment}
`;

const MUTATE_SET_CHECKLIST_ON_TICKET = gql`
  mutation SetChecklistOnTicketWorkflowState(
