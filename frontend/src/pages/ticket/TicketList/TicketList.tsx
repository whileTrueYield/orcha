...
import { TicketCreateModal } from "../TicketCreate/TicketCreateModal";
...
export const TicketList: React.FC = () => {
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {tickets.map((ticket, index) => (
                <TicketListRow
                  ...
                  key={ticket.id}
                  index={index}
                  url={urlResolver.ticket.view(orgId, ticket.id)}
  ...
  // TODO: fix the project select to show the project's full path
  return (
    <div className="mx-auto mb-8 flex w-full flex-col justify-start sm:mt-6">
      <div className="mx-4 mt-6 flex flex-1 flex-row sm:mx-0 sm:mt-0">
        <div className="flex-1">
          <ProjectSelect
            inputClassName="sm:rounded-l-none rounded-r-none"
            onChange={(project) =>
              setListFilter({
                ...ticketFilter,
                recordSets: {
                  ...ticketFilter.recordSets,
                  projects: project
                    ? [{ label: "fix-me project name", id: project.id }]
                    : [],
                },
              })
            }
        ...
        <div className="hidden items-center rounded-r-md border border-l-0 border-gray-300 bg-gray-50 px-4 py-2 shadow-sm sm:flex">
          <CheckboxGroup
            checked={ticketFilter.recursive}
            id="include_sub_projects"
            label="Include Sub Projects"
            onChange={(event) =>
              setListFilter({
                ...ticketFilter,
        ...
        <div className="flex h-full items-center rounded-r-md border border-l-0 border-gray-300 bg-gray-50 px-4 py-2.5 shadow-sm sm:hidden">
          <CheckboxGroup
            checked={ticketFilter.recursive}
            label="Recursive"
            onChange={(event) =>
