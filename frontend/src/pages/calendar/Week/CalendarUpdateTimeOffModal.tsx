...
interface Props extends ModalProps {
  timeOff: TimeOff;
  onChange: (startDate: Date, stopDate: Date) => void;
}

export const CalendarUpdateTimeOffModal: React.FC<Props> = (props) => {
  const { timeOff } = props;

  const formContext = useForm<FormSchema>({
    resolver: yupResolver(schema),
    defaultValues: {
      startAt: startOfDay(new Date(timeOff.startAt)),
      stopAt: startOfDay(new Date(timeOff.stopAt)),
      startTime: format(new Date(timeOff.startAt), "p"),
      stopTime: format(new Date(timeOff.stopAt), "p"),
    },
  });
