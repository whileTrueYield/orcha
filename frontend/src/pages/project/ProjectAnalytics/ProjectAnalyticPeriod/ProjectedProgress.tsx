...
import {
  ProjectGoalProgress,
  QueryProjectedGoalProgressArgs,
} from "types/graphql";
...
export const ProjectedProgress: React.FC<Props> = (props) => {
  ...
  const { data, error, loading } = useQuery<
    QueryReturnValue["projectedGoalProgress"],
    QueryProjectedGoalProgressArgs
  >(GET_PROJECTED_GOAL_PROGRESS, {
    variables: {
      projectId,
      startDate,
      stopDate,
  ...
  const goals = useMemo(
    () =>
      cumulateProjectProgress(
        sortProjectsByAncestry(data?.projectedGoalProgress)
      ),
    [data?.projectedGoalProgress]
  );

  // const goals = data ? data.projectedGoalProgress : [];
  ...
  if (!data?.projectedGoalProgress.length) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 py-10">
      ...
      return (
        <>
          <td className={`${tdClass} min-w-[18rem]`}>
            <ProjectProgressBar
              goal={goal}
              title="Current 2 Weeks Projections"
              labelPreviousWork="previous period"
              labelLeftToDo="projected left to do"
              labelProgress="projected progress"
              labelTotal="total"
            />
          </td>
        </>
    ...
    return (
      <tr key={goal.id} className="group">
        <td className={tdClass}>
          <div
            className="absolute top-0 left-0 bottom-0 bg-gray-100"
          ...
          <table className="w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr className="">
                ...
                {/* <th className={thClassText}>Total</th> */}
...
const GET_PROJECTED_GOAL_PROGRESS = gql`
