...
export const CompareValuesNowWidget: React.FC<Props> = (props) => {
  const {
    values,
    chartBy,
    noUnknowns,
  ...
  const { data, keys } = formatForBarChart(
    [
      ...map(values.primary, (e) => ({ ...e, secondary: chartByLabel })),
      ...map(values.secondary, (e) => ({
        ...e,
        secondary: secondaryChartByLabel,
      })),
    ],
    {
      main: chartBy,
      secondary: secondaryChartBy,
      noUnknowns,
    }
  );
