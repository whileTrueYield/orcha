...
import { ReportCreateModal } from "../ReportCreate/ReportCreateModal";
...
export const ReportList: React.FC = (props) => {
  ...
  const [createReportModalVisible, setCreateReportModalVisibility] = useState(
    urlQuery.get("create") === "true"
  );

  const searchElt = useSlashForSearch();

  const { data, loading, error } = useQuery<QueryReturnValue["reports"]>(
    GET_REPORTS,
    {
      variables: {
  ...
  const loadingList = (
    <div className="col-span-6">
      <div className="flex h-64 w-full flex-col items-center justify-center">
        ...
        <p className="mt-4 tracking-wide text-gray-400">Loading Reports...</p>
      </div>
    </div>
  );
  ...
  const emptyList = (
    <div className="col-span-6">
      <EmptyState
        title="No Reports."
        subTitle="Get started by creating a new report."
      >
        <Button
          className="my-4"
          type="button"
  ...
  const reports = (data?.reports ? data.reports.nodes : []) as Report[];
  const total = data?.reports ? data.reports.totalCount : 0;
  ...
  const reportList = reports.map((report, index) => (
    <li key={report.id}>
      <ReportListRow
        report={report}
        index={index}
        url={urlResolver.report.view(orgId, report.id)}
      />
    </li>
  ));
  ...
  return (
    <div className="mx-auto mb-8 flex w-full flex-col justify-start sm:mt-6">
      <ReportCreateModal
        ...
        onClose={() => setCreateReportModalVisibility(false)}
      />
        ...
        <div className="flex-1">
          ...
          >
            ...
            <input
              ...
              placeholder={`Search report... (press "/" to focus)`}
...
const GET_REPORTS = gql`
  query GetReports($first: Int!, $search: String, $offset: Int) {
    reports(first: $first, search: $search, offset: $offset) {
      totalCount
      pageInfo {
        pageNumber
        hasNextPage
      ...
      nodes {
        ...
        name
        stage
        ...ReportListRowFragment
      }
    }
  }
  ${ReportListRow.fragments.ReportListRowFragment}
`;
