/**
 * Mutation: deleteRole — deactivate a role.
 */
  ...
  t.prismaField({
    ...
    resolve: async (query, _root, args, ctx) => {
      ...
      if (role.type === RoleType.OWNER && (ctx.me as AuthRoleContext).roleType !== RoleType.OWNER) {
        throw new GraphQLError("Only an owner can deactivate another owner", { extensions: { code: "BAD_USER_INPUT" } });
      }

      if (role.id === (ctx.me as AuthRoleContext).roleId) {
        throw new GraphQLError("You cannot deactivate yourself", { extensions: { code: "BAD_USER_INPUT" } });
      }
