...
@Resolver(Role)
export class ReactivateRoleResolver {
  ...
  async reactivateRole(
    ...
    if (role.type === RoleType.OWNER && ctx.me.roleType !== RoleType.OWNER) {
      throw new UserInputError("Only an owner can reactivate another owner");
    }

    if (role.id === ctx.me.roleId) {
      throw new UserInputError("You cannot reactivate yourself");
    }
